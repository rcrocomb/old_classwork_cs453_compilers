...
# stack offset after '__var_add_result_022__' is 12
# stack offset after '__var_gt_result_021__' is 13
# stack offset after '__var_div_result_020__' is 20
# stack offset after '__var_add_result_018__' is 24
# stack offset after '__var_add_result_017__' is 28
# stack offset after '__var_lt_result_015__' is 29
# stack offset after '__var_gt_result_012__' is 30
# stack offset after '__var_div_result_011__' is 36
# stack offset after '__var_add_result_009__' is 40
...
# OP_ADD | i | j | __var_add_result_009__
    lw $t0, 72($sp)
    lw $t1, 68($sp)
    add $t2, $t0, $t1
    sw $t2, 36($sp)
...
# OP_DIV | __var_add_result_009__ | __var_intcon_010__ | __var_div_result_011__
    lw $t0, 36($sp)
    lw $t1, __var_intcon_010__
    div $t2, $t0, $t1
    sw $t2, 32($sp)

# OP_ASSIGN | __var_div_result_011__ | No src2 | k
    lw $t0, 32($sp)
    sw $t0, 64($sp)

# OP_DUMMY | No src1 | No src2 | k
# OP_DUMMY | No src1 | No src2 | i
# OP_GT | k | i | __var_gt_result_012__
    lw $t0, 64($sp)
    lw $t1, 72($sp)
    sgt $t2, $t0, $t1
    sb $t2, 29($sp)

# OP_COND_BRANCH | __var_gt_result_012__ | __label_else_004__ | No dest
    lb $t2, 29($sp)
    beqz $t2, __label_else_004__

    nop
# OP_DUMMY | No src1 | No src2 | __var_intcon_013__
# OP_ASSIGN | __var_intcon_013__ | No src2 | a
...
# OP_DUMMY | No src1 | No src2 | k
# OP_ADD | a | k | __var_add_result_018__
    lw $t0, 60($sp)
    lw $t1, 64($sp)
    add $t2, $t0, $t1
    sw $t2, 20($sp)
...
# OP_DIV | __var_add_result_018__ | __var_intcon_019__ | __var_div_result_020__
    lw $t0, 20($sp)
    lw $t1, __var_intcon_019__
    div $t2, $t0, $t1
    sw $t2, 16($sp)

# OP_ASSIGN | __var_div_result_020__ | No src2 | b
    lw $t0, 16($sp)
    sw $t0, 56($sp)

# OP_DUMMY | No src1 | No src2 | b
# OP_DUMMY | No src1 | No src2 | j
# OP_GT | b | j | __var_gt_result_021__
    lw $t0, 56($sp)
    lw $t1, 68($sp)
    sgt $t2, $t0, $t1
    sb $t2, 12($sp)

# OP_COND_BRANCH | __var_gt_result_021__ | __label_else_008__ | No dest
    lb $t2, 12($sp)
...
# OP_DUMMY | No src1 | No src2 | b
# OP_ADD | res | b | __var_add_result_022__
    lw $t0, 52($sp)
    lw $t1, 56($sp)
    add $t2, $t0, $t1
    sw $t2, 8($sp)

# OP_ASSIGN | __var_add_result_022__ | No src2 | res
    lw $t0, 8($sp)
    sw $t0, 52($sp)
...
# OP_DUMMY | No src1 | No src2 | res
# OP_DUMMY | No src1 | No src2 | k
# OP_ADD | res | k | __var_add_result_023__
    lw $t0, 52($sp)
    lw $t1, 64($sp)
    add $t2, $t0, $t1
    sw $t2, 4($sp)

# OP_ASSIGN | __var_add_result_023__ | No src2 | res
    lw $t0, 4($sp)
    sw $t0, 52($sp)
...
# OP_ADD | a | __var_intcon_016__ | __var_add_result_017__
    lw $t0, 60($sp)
    lw $t1, __var_intcon_016__
    add $t2, $t0, $t1
    sw $t2, 24($sp)

# OP_ASSIGN | __var_add_result_017__ | No src2 | a
    lw $t0, 24($sp)
    sw $t0, 60($sp)
...
# OP_LT | a | __var_intcon_014__ | __var_lt_result_015__
    lw $t0, 60($sp)
    lw $t1, __var_intcon_014__
    slt $t2, $t0, $t1
    sb $t2, 28($sp)

# OP_FALSE_COND_BRANCH | __var_lt_result_015__ | __label_for_top_006__ | No dest
    lb $t2, 28($sp)
...
# OP_ADD | j | __var_intcon_024__ | __var_add_result_025__
    lw $t0, 68($sp)
    lw $t1, __var_intcon_024__
    add $t2, $t0, $t1
    sw $t2, 0($sp)

# OP_ASSIGN | __var_add_result_025__ | No src2 | j
    lw $t0, 0($sp)
    sw $t0, 68($sp)
